...
clc
close all
para.sensitivity_PRC_ReadoutPerNM=102%279.83%167.5%[76.9865632572905];
...
pa='..\bin\'
FN=dir([pa 'test_z_actuator_frequency_20150618141124.txt'])
...
z_piezo_NM=data(:,1);
prc_readout=data(:,2);
prc_readout=prc_readout(prc_readout~=0);
prc_readout=prc_readout(300:end);
...
th=(max(z_tip_NM_base)-mean(z_tip_NM_base))*0.051+mean(z_tip_NM_base);
ind=z_tip_NM_base>th;
z_tip_NM_base=z_tip_NM_base(ind);
z_piezo_NM=z_piezo_NM(ind);
z_piezo_NM=z_piezo_NM-z_piezo_NM(1);
...
L=find(indent_force==max(indent_force))-1;
...
% D=D(1:end-2);
% F=F(1:end-2);
E_sample=fit_youngs_modulus_linear_show_simulation(D,F,para,0.3,0.9,fn)
